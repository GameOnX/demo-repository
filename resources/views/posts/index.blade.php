            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <div class="flex justify-between items-center">
                        <h3 class="text-2xl font-bold text-gray-900">All Posts</h3>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach ($posts as $post)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-900">{{ $post->created_at->format('j M Y, g:i a') }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <a href="{{ route('posts.show', $post) }}" class="text-indigo-600 hover:text-indigo-900">View</a>
                                            @can('update', $post)
                                                <a href="{{ route('posts.edit', $post) }}" class="ml-4 text-indigo-600 hover:text-indigo-900">Edit</a>
                                            @endcan
                                            @can('delete', $post)
                                                <form class="inline-block ml-4" method="POST" action="{{ route('posts.destroy', $post) }}">
                                                    @csrf
                                                    @method('delete')
